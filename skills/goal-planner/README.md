...
**스킬 이름은 계속 `goal-planner`입니다.** 2.0.0은 이번 개선 패키지의 배포 번호이며 특정 모델 버전이 아닙니다. 계획 파일 작성과 프로젝트 실행은 구분됩니다. 이 패키지는 모델·API·유료 서비스를 자동으로 호출하거나 목표를 백그라운드에서 실행하지 않습니다.
...
인덱스 도구와 테스트는 Python 3.10 이상을 대상으로 작성했으며 이번 검증 환경은 Python 3.13.5입니다. 계획 작성만 할 때 Python 실행은 필수가 아닙니다.
...
31개 자동 검사 통과는 인덱스 코드와 정적 패키지 검사 결과입니다. 실제 모델 행동 시나리오 18개는 미실행 상태입니다. 모델별 품질 향상·토큰 절감·호스트 연동 성공을 측정한 것으로 해석하지 마세요. 상세 결과는 [VALIDATION.md](VALIDATION.md)에 있습니다.
