...
**스킬 이름은 계속 `goal-planner`입니다.** 2.4.0은 스킬 패키지 버전이며 특정 모델 버전이 아닙니다. 목표 선택을 위임받으면 프로젝트 목적과 현재 진행 상황을 바탕으로 중요한 병목·역량 부족·기술적 결정을 해소하는 이정표를 정합니다. 계획 파일 작성과 프로젝트 실행은 구분됩니다. 이 패키지는 모델·API·유료 서비스를 자동으로 호출하거나 목표를 백그라운드에서 실행하지 않습니다.
...
실행 계약은 Core를 항상 포함하고 Direction·Research·Retrieval은 실제로 필요할 때, Persistence는 기록이 승인됐을 때만 포함합니다. 알려진 작은 수정은 Core만으로 작성할 수 있으며, 필요한 블록은 계획에 전문을 넣어 외부 스킬 경로 없이도 수행할 수 있게 합니다. 계획 이후 새로운 문제가 드러나면 기존 권한 안에서 대응하며 상세 블록의 부재를 중단 사유로 삼지 않습니다.

중요한 지식 공백이 남고 독립적인 메인 작업·지원 도구·기존 권한과 공유 예산이 있으면 제한된 질문의 연구 위임을 기본으로 합니다. 매번 사용자의 별도 연구자 지시를 기다리지 않습니다. 메인은 독립 작업을 계속하고, 결과에 의존하는 결정 전에 연구를 읽습니다. 승인된 연구는 `docs/researches/`에 완성된 노트로 저장한 뒤 결론·경로를 알리며, 코드 변경에 따른 재검토와 적용·거절·보류 이유를 남깁니다. 연구도 공유 예산을 쓰고 종료·중단 시 남은 작업을 정리합니다. 병행 조건이나 기록 권한이 없으면 필요한 연구의 순차 수행·세션 반환으로 대응합니다. 계획 작성이 실제 연구를 시작하지는 않습니다.

승인된 결과·제약·외부 계약·권한·예산 안의 필요한 접근법 변경은 계속 수행하고, 새 합의에 의존하는 단계만 보류합니다. 스킬이나 작업 지침 때문에 멈추거나 확인을 요청할 때는 실제로 읽은 파일·해당 문장·적용 이유를 설명하여 명시적 요구와 에이전트의 해석을 구분합니다.
...
자동 검사는 인덱스 코드와 정적 패키지 구조를 확인합니다. 행동 평가 정의는 44개이며 자동 검사로 실행되지 않습니다. 별도 표본과 실제 장기 실행·호스트 연동·모델 간 비교는 구분하여 [VALIDATION.md](VALIDATION.md)에 기록합니다.
