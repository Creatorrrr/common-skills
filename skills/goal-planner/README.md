...
**스킬 이름은 계속 `goal-planner`입니다.** 2.3.0은 스킬 패키지 버전이며 특정 모델 버전이 아닙니다. 목표 선택을 위임받으면 프로젝트 목적과 현재 진행 상황을 바탕으로 중요한 병목·역량 부족·기술적 결정을 해소하는 이정표를 정합니다. 계획 파일 작성과 프로젝트 실행은 구분됩니다. 이 패키지는 모델·API·유료 서비스를 자동으로 호출하거나 목표를 백그라운드에서 실행하지 않습니다.
...
중요한 결정에 필요한 연구는 지원되는 실행 환경에서 제한된 질문으로 위임하도록 인계합니다. 메인은 연구 결과가 달라져도 가치가 남는 작업을 계속하고, 결과에 의존하는 결정 전에 연구를 읽습니다. 승인된 연구는 `docs/researches/`에 완성된 노트로 저장한 뒤 결론·경로를 알리며, 코드 변경에 따른 재검토와 적용·거절·보류 이유를 남깁니다. 연구도 공유 예산을 쓰고 종료·중단 시 남은 작업을 정리합니다. 병렬 기능이나 기록 권한이 없으면 필요한 연구의 순차 수행·세션 반환으로 대응합니다. 계획 작성이 실제 연구를 시작하지는 않습니다.

유효한 누적 실패·개선 정체·핵심 가정의 반증이 있으면 현재 방향의 지속과 구조적 전환을 비교합니다. 목표·세션이 달라도 같은 가정의 시도를 함께 검토하며, 반복 실행 오류를 독립적인 반증으로 세지 않습니다. 사용자 결과·명시된 제약은 보존하되 현재 알고리즘·구조는 재검토할 수 있습니다. 근거와 감당 가능한 실패 비용이 정당화하면 큰 변경도 선택하며, 현재 목표가 성립하지 않을 때 필요한 수정안을 제안할 수 있습니다. 이 판단이 새 권한·예산을 부여하거나 과거 실패를 성공으로 바꾸지는 않습니다.
...
자동 검사는 인덱스 코드와 정적 패키지 구조를 확인합니다. 행동 평가 정의는 40개이며 자동 검사로 실행되지 않습니다. 별도 표본과 실제 장기 실행·호스트 연동·모델 간 비교는 구분하여 [VALIDATION.md](VALIDATION.md)에 기록합니다.
