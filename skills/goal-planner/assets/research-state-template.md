<!-- Optional resume snapshot in an existing authorized log/path. One writer; use the existing atomic/versioned save mechanism. Long history stays in linked records. Do not initialize directories or change Git state solely to fill this template. -->
# Research state

- Program ID:
- Program state: active | paused | stopped | completed
- Envelope / authorization reference: [reference is not new authorization]
- Updated: [actual time/zone or unknown]
- Writer / state revision:
- Current milestone / original criteria reference:
- Milestone outcome: pending | pass | fail | blocked | not_run
- Closed milestone outcomes and preserved criteria: [links or concise entries]
- Current experiment / hypothesis / protocol version:
- Current beliefs / supporting and contrary evidence: [brief, scoped]
- Best verified artifact / claim scope / evidence:
- Active candidate / last verified state:
- Workspace / code / relevant dirty-state / config / data / evaluator IDs:
- Evaluation exposure and selection history: [link or short list]

## Shared resources
| Resource and unit | Authorized cap/basis | Actual used | In-flight reserved | Remaining/unknown | Evidence |
|---|---|---|---|---|---|
| | | | | | |
<!-- Do not double-count reservations as final charges. Unknown is not zero; keep old spending on cap changes. -->

## Owned work
| Task ID / owner | Scope / write path | Last observed actual state | Reserved resource | Next reconciliation |
|---|---|---|---|---|
| | | | | |

## Resume
- Next decision / discriminating action:
- Required evidence / blocker / resume condition:
- Files, task state and resource accounting to recheck:
- Already completed work not to repeat:
- Pending persistence or cleanup:
<!-- On restart compare actual state, not just this snapshot. No unsupported automatic restart is implied. -->
