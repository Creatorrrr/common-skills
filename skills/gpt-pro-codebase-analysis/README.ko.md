...
핵심 변경은 **검토한 입력 범위와 실제 전송 내용을 일치시키고, 생성 완료와 분석 검증 완료를 구분하는 것**입니다. 기본 모델 변경에 따른 효과와 구현 수정 효과를 혼동하지 않도록 기본 모델은 `gpt-5.6-sol`로 유지했습니다. Astra는 명시적으로 선택할 수 있습니다.
...
| 항목 | v2 동작 |
|---|---|
| 모델 | 기존 `gpt-5.6-sol` 유지, `gpt-6-astra` 명시 선택 지원 |
| 추론 모드 | `auto`; Sol은 Pro로 해석하고 Astra에는 mode 필드를 보내지 않음 |
| 추론 강도 | 기존 비교 기준인 `high` 유지 |
...
첨부 원본의 오프라인 테스트 80개를 재실행했습니다. 저장소 반영 과정에서 읽기 권한 오류를 바이너리로 잘못 분류해 전체 준비를 성공 처리하는 문제를 수정하고 회귀 테스트를 추가했습니다. 기존 Sol Pro/high 검사는 유지합니다. 최종 실행 수와 로그는 [검증 기록](validation/VALIDATION.md)에 있습니다.
