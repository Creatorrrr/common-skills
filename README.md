...
Use version **2.4.0** to create, revise, or review outcome-first long-running goals, `GOAL_PLAN.md` files, and execution prompts for Codex or Claude Code. Delegated goal selection connects project purpose and ongoing work to a consequential milestone. Valid failure patterns, stalled improvement or a refuted assumption trigger comparison of continuation and structural alternatives. Continue necessary method changes within the approved outcome and resources. Research delegation is the default when a material gap, independent main work, supported tools and existing authority/resources permit it. Handoffs embed Core plus only applicable direction, research, retrieval and persistence blocks. The skill preserves explicit constraints and separates checkpoints from final completion; planning alone does not activate execution.
