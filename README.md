...
Use version **2.3.0** to create, revise, or review outcome-first long-running goals, `GOAL_PLAN.md` files, and execution prompts for Codex or Claude Code. Delegated goal selection connects the project purpose and ongoing work to a consequential milestone. Valid accumulated failures, stalled improvement, or a refuted central assumption trigger comparison of incremental continuation and structural alternatives; larger changes can be selected when evidence and bounded downside justify them. Decision-relevant research can run alongside independent authorized work, with shared limits, completed research notes, and main-session review before dependent decisions. The skill preserves explicit constraints, distinguishes checkpoints from goal completion, and keeps necessary goal-revision proposals separate from automatic activation. Planning and execution remain separate.
