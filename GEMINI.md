...
- `skills/goal-planner/SKILL.md`
  - Use version 2.3.0 when the user wants to create, revise, or review an outcome-first long-running goal, `GOAL_PLAN.md`, or execution prompt. Connect project purpose and ongoing work to a consequential milestone. When valid failure patterns, stalled improvement or a refuted assumption warrant it, compare continuation with structural alternatives while preserving explicit constraints and authorization. Plan bounded research alongside useful independent work, with evidence return and review before dependent decisions. Distinguish checkpoint completion from final completion. Verify runtime support before using `/goal` syntax or assuming parallel execution.
