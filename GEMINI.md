...
- `skills/goal-planner/SKILL.md`
  - Use version 2.4.0 when the user wants to create, revise, or review an outcome-first long-running goal, `GOAL_PLAN.md`, or execution prompt. Connect project purpose and ongoing work to a consequential milestone; continue justified method changes within the approved outcome and resources. Default to bounded research delegation when a material gap, independent main work, supported tools and existing authority/resources permit it. Embed Core plus only applicable contract blocks, preserving explicit constraints and direct outcome evidence. Verify runtime support before using `/goal` syntax or assuming parallel execution.
