...
- `skills/goal-planner/SKILL.md`
  - Use version 3.2.1 when the user wants to create, revise, or review agent goals, `GOAL_PLAN.md`, or execution handoffs for research and development. Default to one `finite-goal`; use `research-program` when continuing research and successor selection are delegated. Connect mission, finite milestones, and experiments while preserving original verdicts, best verified artifacts, evaluation history, and shared resources across successors and resumptions. Use bounded delegation for independent research, implementation or verification when it improves time or quality within supported tools, existing authority and shared resources. Embed Core plus only applicable Program, Experiment, Direction, Research, Retrieval, and Persistence blocks. Verify runtime support before using `/goal` syntax or assuming parallel execution or automatic restart.
  - For consequential comparisons, reconcile declared variants with actual construction/behavior, distinguish diagnosis from promotion confirmation, and review raw evidence before causal claims. Optional preflight consistency is not scientific validity; small corrections do not require experimental infrastructure. Candidate portfolio checks and handoff assembly are optional; Astra host notes do not change model settings.
